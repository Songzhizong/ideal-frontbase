...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "sheet-overlay fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm",
      className,
    )}
    {...props}
  />
...
const sheetVariants = cva(
  "sheet-content fixed z-50 flex flex-col bg-background shadow-2xl rounded-[20px] border border-border/60 overflow-hidden",
  {
    variants: {
      side: {
        left:
          "inset-y-5 left-5 w-72",
        right:
          "inset-y-5 right-5 w-72",
        top: "inset-x-5 top-5 h-auto",
        bottom:
          "inset-x-5 bottom-5 h-auto",
      },
    },
    defaultVariants: {
      side: "right",
