...
function runTypecheckFallback() {
  if (isAll) {
    console.warn("Turbo 在执行全量 typecheck 时崩溃，自动回退到 pnpm 递归 typecheck。")
    return spawnSync(
      "pnpm",
      ["-r", "--if-present", "--filter=./apps/*", "--filter=./packages/*", "run", "typecheck"],
      {
        stdio: "inherit",
      },
    )
  ...
  if (appName) {
    console.warn(
      `Turbo 在执行 ${appName} 的 typecheck 时崩溃，自动回退到 pnpm --filter typecheck。`,
    )
    return spawnSync(
      "pnpm",
      [
...
if (isAll) {
  ...
  if (!apps.includes(appName)) {
    console.error(`应用 "${appName}" 不存在。当前可用应用：${apps.join(", ")}`)
    process.exit(1)
  }
...
if (task === "typecheck" && (result.status ?? 1) === 101) {
  const fallback = runTypecheckFallback()
