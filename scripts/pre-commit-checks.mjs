...
function hasThemeTokenRelevantChange(stagedFiles) {
  return stagedFiles.some((file) => {
    if (file.startsWith("packages/theme-system/tokens/")) {
      return true
    }

    return (
      file === "packages/theme-system/scripts/generate-token-artifacts.mjs" ||
      file === "packages/theme-system/styles/variables.css" ||
      file === "packages/theme-system/styles/theme.css" ||
      file === "packages/theme-system/styles/variables.generated.css" ||
      file === "packages/theme-system/styles/theme.generated.css"
    )
  })
}

function hasThemeSystemChange(stagedFiles) {
  return stagedFiles.some((file) => file.startsWith("packages/theme-system/"))
}
...
if (hasThemeTokenRelevantChange(stagedFiles)) {
  console.log("附加检查：检测到主题 token 变更，执行 tokens:check...")
  run("pnpm", ["tokens:check"])
}

if (hasThemeSystemChange(stagedFiles)) {
  console.log("附加检查：检测到主题系统变更，执行 theme-system 单测...")
  run("pnpm", ["-C", "packages/theme-system", "test"])
}
