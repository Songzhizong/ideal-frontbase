 ...
 */
const getPermissionIdents = async (): Promise<PermissionIdents> => {
 ...
 * @example
 * const { data: permissions, isLoading, refetch } = usePermissionIdents({ enabled: false })
 * // 手动获取权限信息
 * const handleRefresh = () => refetch()
 */
export const usePermissionIdents = (options?: { enabled?: boolean }) => {
  return useQuery({
    queryKey: ["auth", "permissions"],
    queryFn: getPermissionIdents,
 ...
 * 可在非React环境中使用
 */
export const fetchPermissionIdents = getPermissionIdents
