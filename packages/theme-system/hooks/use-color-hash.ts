...
export interface SemanticChartColors {
  positive: string
  negative: string
  neutral: string
  warning: string
...
export function useSemanticChartColors(): SemanticChartColors {
  ...
  return useMemo(() => {
    const semantic = preset?.schemes[mode]?.charts?.semantic
    return {
      positive: semantic?.positive ?? "hsl(var(--success))",
      negative: semantic?.negative ?? "hsl(var(--error))",
      neutral: semantic?.neutral ?? "hsl(var(--muted-foreground))",
      warning: semantic?.warning ?? "hsl(var(--warning))",
    }
  }, [preset, mode])
}
