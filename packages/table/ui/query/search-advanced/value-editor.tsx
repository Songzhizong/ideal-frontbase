...
import { areValuesEqual } from "../filters/value-utils"
import type { DataTableQueryField } from "../types"
import type { SearchOptionEntry } from "./utils"

export interface DataTableQueryAdvancedValueEditorProps<TFilterSchema, TFieldId extends string> {
...
export function DataTableQueryAdvancedValueEditor<TFilterSchema, TFieldId extends string>({
  activeField,
  fieldValue,
  ...
  onConfirmRange,
  i18n: i18nOverrides,
}: DataTableQueryAdvancedValueEditorProps<TFilterSchema, TFieldId>) {
  ...
  useEffect(() => {
    if (activeField?.kind !== "number-range") return
  ...
  if (
    ...
    activeField.kind === "custom"
  ) {
    return null
  }

  if (activeField.kind === "boolean" || activeField.kind === "select") {
    return (
