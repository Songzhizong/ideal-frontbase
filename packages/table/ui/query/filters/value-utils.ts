...
export function getDefaultClearedValue(kind: FilterType): unknown {
  if (kind === "text") return ""
  return null
}

export function isEmptyValue(value: unknown, kind: FilterType): boolean {
  if (value == null) return true
  if (kind === "text" && typeof value === "string") return value.trim() === ""
  if (kind === "multi-select") return !Array.isArray(value) || value.length === 0
  ...
  if (kind === "date") return !isDateValue(value)
