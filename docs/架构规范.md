...
- **Shadcn UI**: Always use components from `@/packages/ui`. Do not invent new UI styles unless necessary.
...
在 `src/packages/api-core/api-client.ts` 中实现 Ky 实例配置，替代传统的 Axios Interceptors。
