...
为了防止权限标识符拼写错误并提升开发体验，建议在每个应用的 `apps/<app-name>/src/config/permissions/` 目录中维护权限常量，并根据功能模块进行拆分（如 `users.ts`）。`Permission` 类型统一由 `@/packages/auth-core` 提供。

```typescript
// apps/<app-name>/src/config/permissions/users.ts
export const USER_PERMISSIONS = {
  USERS_READ: "users:read",
  // ...
} as const;

// apps/<app-name>/src/config/permissions/index.ts
...
```typescript
// apps/<app-name>/src/routes/_authenticated.users.tsx
...
const columns = useMemo(() => {
  const base = [...];
  if (hasPermission(PERMISSIONS.USERS_VIEW_PHONE)) {
...
```typescript
import { useHasPermission } from "@/packages/auth-core"
...
- [`auth-store.ts`](../packages/auth-core/auth-store.ts) - 核心权限 Store (含 Set 优化)
- [`use-has-permission.ts`](../packages/auth-core/use-has-permission.ts) - 性能优化专用 Hook
- [`permission-guard.tsx`](../apps/admin/src/features/core/auth/components/permission-guard.tsx) - 声明式权限组件（其他应用按 `apps/<app-name>/src/...` 对应替换）
- [`auth-button.tsx`](../packages/auth-core/auth-button.tsx) - 权限按钮组件
